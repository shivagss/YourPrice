...
import com.aviary.android.feather.sdk.widget.ImageViewTiltiShiftTouch.OnTiltShiftDrawListener;
import com.aviary.android.feather.sdk.widget.ImageViewTiltiShiftTouch.TiltShiftDrawMode;
...
public class TiltShiftPanel extends AbstractContentPanel implements OnTiltShiftDrawListener, OnDrawableChangeListener, OnCheckedChangeListener {
	...
	public TiltShiftPanel ( IAviaryController context, ToolEntry entry ) {
		super( context, entry );
		mToolAction = new ToolActionVO<String>();
	}
	...
	public void onActivate() {
		...
		mCanvasPreview = new Canvas(mPreview);
		mPaintPreview = new Paint();
	...
	public void onCheckedChanged( AviaryHighlightImageButton button, boolean isChecked, boolean fromUser ) {
		...
		if ( mTiltShiftMode == TiltShiftMode.Radial ) {
			...
			attrs.put("style", "rectangle");
		}
		getContext().getTracker().tagEventAttributes( ToolLoaderFactory.Tools.FOCUS.name().toLowerCase( Locale.US ) + ": style_selected", attrs );
	}
	...
	protected void onComplete( Bitmap bitmap, MoaActionList actions ) {
		...
		if ( mTiltShiftMode == TiltShiftMode.Radial ) mTrackingAttributes.put( "style", "circle" );
		else mTrackingAttributes.put( "style", "rectangle" );
	...
	class BackgroundDrawThread extends Thread {
		...
		synchronized public void drawStart( float points[], TiltShiftDrawMode type, float radius, float angle, float left, float top, float right, float bottom ) {
			...
			mToolAction.setValue(type == TiltShiftDrawMode.Radial ? "circle" : "rectangle");
			...
			while ( running ) {
				...
				if ( mQueue.size() > 0 && !isInterrupted() ) {
					...
					while ( element.size() > 0 || !element.isCompleted() ) {
						...
						mFilter.draw( radius, angle, 0, false, x, y );
	...
	private void postInvalidate() {
		if(isActive() && null != mPreview && !mPreview.isRecycled() && null != mCanvasPreview && null != mPaintPreview){
			mLogger.info("postInvalidate");
			mPaintPreview.setColor(mPreview.getPixel(0, 0));
			mCanvasPreview.drawPoint(0, 0, mPaintPreview);
