...
import com.aviary.android.feather.sdk.widget.ImageViewDrawableOverlay.OnDrawableEventListener;
...
public class TextPanel extends AbstractContentPanel implements OnDrawableEventListener, OnEditorActionListener, OnItemsScrollListener, OnKeyListener {
			...
			if (hv != null) {
				if (hv.getContent() instanceof EditableDrawable) {
					...
					if (editable != null) {
						...
						if (editable.isTextHint()) return false;
						if (null == editable.getText()) return false;
	...
	private MoaActionList onApplyCurrent( final DrawableHighlightView hv ) {
		...
		if ( hv != null ) {
			...
			TextFilter filter = (TextFilter) ToolLoaderFactory.get(ToolLoaderFactory.Tools.TEXT);
			...
			mToolAction.setValue((String) editable.getText());
	...
	public void onMove(final DrawableHighlightView view) {
		if (view.getContent() instanceof EditableDrawable) {
			if (((EditableDrawable) view.getContent()).isEditing()) {
				endEdit(view);
			}
		}
	}
