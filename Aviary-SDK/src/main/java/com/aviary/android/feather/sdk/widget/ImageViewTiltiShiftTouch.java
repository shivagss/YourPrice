...
public class ImageViewTiltiShiftTouch extends ImageViewTouch {
	...
	protected void init( Context context, AttributeSet attrs, int defStyle ) {
		...
		mManager = new EasingManager( new EasingManager.EasingCallback() {
	...
	private void initializeTouch( float left, float top, float right, float bottom ) {
		...
		if ( null != mPointCloud && mPointCloudEnabled ) {
			...
			mManager.stop();
			mManager.start( Quad.class, EasingManager.EaseType.EaseOut, inner, mPointCloudOuterRadius + 100, mPointWaveDuration, 100 );
		}
	}
