...
import com.aviary.android.feather.sdk.widget.AviaryAdapterView.OnItemClickListener;
...
public class CropPanel extends AbstractContentPanel implements com.aviary.android.feather.sdk.widget.AviaryGallery.OnItemsScrollListener, OnItemClickListener {
	...
	private void setCustomRatio ( double aspectRatio, boolean isFixed ) {
		mTrackingAttributes.put( "aspect", String.valueOf( aspectRatio ) );
	...
	protected View generateContentView ( LayoutInflater inflater ) {
		@SuppressLint ("InflateParams") View view = inflater.inflate( R.layout.aviary_content_crop, null );
		return view;
	}
	...
	public void onItemClick ( AviaryAdapterView<?> parent, View view, int position, long id ) {
		...
		if( ! mStrictPolicy && ! mNonInvertOptions.contains( position ) ) {
			...
			mTrackingAttributes.put( "aspect", String.valueOf( currentAspectRatio ) );
		}
	}
	...
	public void onScrollFinished ( AviaryAdapterView<?> parent, View view, int position, long id ) {
		...
		mLogger.info( "onScrollFinished: %d --> %d", mSelectedPosition, position );
