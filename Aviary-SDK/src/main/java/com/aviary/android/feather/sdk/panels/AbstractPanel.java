...
public abstract class AbstractPanel {
	...
	// if you want to use more custom constants to be used
	// beside the one used here, just start from this value.
	// We'll not use constants with the value > than this one ever.
	public static final int LAST_VALID_MESSAGE = 999;

	/** every panel should result this object */
	protected final EditToolResultVO mEditResult;

	/**
	 * If the current panel implements {@link com.aviary.android.feather.sdk.panels.AbstractContentPanel} this listener is used
	 * by the {@link com.aviary.android.feather.sdk.AviaryMainController} to hide the main
	...
	public static interface OnApplyResultListener {
		 ...
		 * @param actions
		 */
		void onComplete(Bitmap result, EditToolResultVO actions);
	}

	/**
	 * The listener interface for receiving onHiresError events. The class that is interested
	 * in processing a onHiresError event implements
	 * this interface, and the object created with that class is registered with a
	...
	public boolean hasOption(final String key) {
		return hasOptions() && mOptions.containsKey(key);
	}
	 ...
	 * @param editResult the result generated from the current tool
	 */
	protected void onComplete(Bitmap bitmap, EditToolResultVO editResult) {
		...
		if (mApplyListener != null && isActive()) {
			...
			mApplyListener.onComplete(bitmap, editResult);
	 ...
	 * {@link #onComplete(android.graphics.Bitmap, com.aviary.android.feather.library.vo.EditToolResultVO)} event.
	 */
	protected void onGenerateResult() {
		onGenerateResult( null );
	}

	protected void onGenerateResult( EditToolResultVO resultVO ) {
