...
public class DrawingPanel extends AbstractContentPanel implements OnDrawStartListener, OnDrawPathListener, OnClickListener {
	...
	private Toast makeToast () {
		...
		mEraserDrawable = new PreviewSpotDrawable( this.getContext()
				                                           .getBaseContext() );
	...
	private void updateToast () {
		...
		if( null != mToast && color_pos > - 1 && size_pos > - 1 ) {
			...
			ImageView image = (ImageView) mToast.getView()
					.findViewById( R.id.image );
		...
		switch( which ) {
			...
			case Erase:
				...
				getContext().getTracker()
						.tagEvent( ToolLoaderFactory.Tools.DRAW.name()
								           .toLowerCase( Locale.US ) + ": eraser_selected" );
	...
	public void setPanelEnabled ( boolean value ) {
		...
		mOptionView.findViewById( R.id.aviary_disable_status )
				.setVisibility( value ? View.INVISIBLE : View.VISIBLE );
	}
